...
public class Test {
    ...
    *  四、模式优缺点
       ...
       六。模式总结
       1、建造者模式是将一个复杂对象的创建过程给封装起来，客户只需要知道可以利用对象名或者类型就能够得到一个完整的对象实例，
       而不需要关心对象的具体创建过程。
       2、建造者模式将对象的创建过程与对象本身隔离开了，使得细节依赖于抽象，符合依赖倒置原则。
       可以使用相同的创建过程来创建不同的产品对象。
    *
    * */
    public static void main(String[] args) {
        KFCWaitor waitor = new KFCWaitor();
        MealBuilder builder = new MealA();
        waitor.setBuilder(builder);
        Meal m = waitor.construct();
